...
function MainHostel({cardInfo}){
    ...
    useEffect(()=>{
        ...
        .then((res)=>{
            if(res.status===429){throw new Error("Too many request please try again later")}
            if(!res.ok){throw new Error("network res was not ok"+res.statusText)}
            return res.json()})
        .then((json)=>{return setShowAl(json)})
        .catch(error=>{
